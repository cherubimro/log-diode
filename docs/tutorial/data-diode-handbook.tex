...
\textbf{log-diode is an ingest deployment.} Syslog is produced on the
lower-security network --- the datacenter, where a compromised device may
already be --- and crosses into the higher-security aggregator. So
\srcside{}~=~low security and \dstside{}~=~high security here, and the
property being bought is Bell--LaPadula confidentiality of the collector. The
paper pairs it with a Biba-style integrity argument and write-once read-many
storage on the receiving side, so logs at rest cannot be altered even from
within it \cite{anton2024}.
...
the \dstside{} side of the boundary. A parser that can fault or read out of
...
\begin{lstlisting}[style=shell,caption={DESTINATION side --- start this first}]
...
\begin{lstlisting}[style=conf,caption={DESTINATION host --- \file{/etc/rsyslog.d/10-diode-in.conf}}]
module(load="imudp")
...
\item Rotation means restarting both daemons; do the \dstside{} side first so
...
\dstside{} sees datagrams but emits no syslog &
