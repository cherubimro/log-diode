...
\newcommand{\projectname}{log-diode}
\newcommand{\doctitleshort}{Data Diode Quick Start}
\input{common}
\renewcommand{\amp}{\prog{ld\_amp}}
\renewcommand{\deamp}{\prog{ld\_deamp}}
...
\textbf{log-diode ingests.} Logs are produced on the lower-security network
and cross into the higher-security collector, so here \srcside{} is the
less-trusted side. The Security Operations Centre sees the plant's logs, and
nothing in the corporate world can reach --- or write to --- the plant. That is
a Bell--LaPadula arrangement, and a diode enforces it with optics rather than
with policy.
...
Start the \dstside{} side first, so nothing is lost:

\begin{lstlisting}[style=shell]
# DESTINATION host -- recover, authenticate, re-emit to the local rsyslog
./bin/ld_deamp 6514 127.0.0.1 514 --key 9f2c...4ab1
\end{lstlisting}

Then the \srcside{} side:

\begin{lstlisting}[style=shell]
# SOURCE host -- take syslog on :1514, ship it across the diode
...
and on the \dstside{} host, make rsyslog listen locally for what \deamp{}
re-emits:

\begin{lstlisting}[style=conf,caption={\file{/etc/rsyslog.d/10-diode-in.conf} --- DESTINATION host}]
module(load="imudp")
...
Start at \cmd{--batch 8 --parity 6 --interleave 8}. If the \dstside{} side
...
\begin{lstlisting}[style=conf,caption={\file{/etc/systemd/system/ld-amp.service} --- SOURCE host}]
[Unit]
Description=log-diode amplifier (SOURCE side)
...
The \dstside{} unit is the same shape with
