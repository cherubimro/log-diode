...
%%
%% diode-quickstart.tex -- the short path to a working one-way syslog link.
...
\title{\vspace{-12mm}\textbf{Building a Data Diode: Quick Start}\\[2mm]
       \large A one-way syslog link in an afternoon}
\author{Alin-Adrian Anton \\
        \small \texttt{alin.anton@upt.ro} \\
        \small Politehnica University of Timi\c{s}oara}
\date{\small Companion to \emph{The Data Diode Handbook}\\
      \small Anton, Csereoka, Capot\u{a} \& Cioarg\u{a},
...
\begin{abstract}
\noindent
This is the short road. It gets you from an empty bench to syslog messages
crossing a genuinely unidirectional link, running the formally verified
\amp{} and \deamp{} daemons at each end. Two routes are given: a
...
A data diode carries information from a lower-security network to a
higher-security one and makes the reverse physically impossible. Logs are
the classic application: the Security Operations Centre must see the
plant's logs, but nothing in the corporate world should be able to reach --- or
write to --- the plant. That is a Bell--LaPadula arrangement, and a diode
enforces it with optics rather than with policy.

\begin{figure}[h]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[node distance=15mm]
  \node[host, minimum width=22mm] (src) {Syslog\\sources};
  ...
  \node[host, minimum width=22mm, right=of deamp] (siem) {rsyslog\\SIEM, WORM};

  \draw[dataflow] (src)   -- node[lbl,above]{:1514} (amp);
  \draw[dataflow] (amp)   -- node[lbl,above]{sealed} (diode);
  \draw[dataflow] (diode) -- node[lbl,above]{:6514} (deamp);
  \draw[dataflow] (deamp) -- node[lbl,above]{:514} (siem);
  ...
  \begin{scope}[on background layer]
    \node[netbox, inner sep=3mm, fit=(src)(amp),
          label={[lbl]above:LOW-security network}] {};
    \node[netbox, inner sep=3mm, fit=(deamp)(siem),
          label={[lbl]above:HIGH-security network}] {};
  \end{scope}
\end{tikzpicture}}
...
\item Two GNU/Linux hosts (or two VMs, or two network namespaces), each with a
      ...
      \lowside{} host is \cmd{192.168.10.1} and the \highside{} host is
      \cmd{192.168.10.2}, both on \cmd{enp1s0}.
\item \prog{log-diode} built: \cmd{./tools/build.sh} produces
      \cmd{bin/ld\_amp} and \cmd{bin/ld\_deamp}. This needs GNAT 14.2.0 and
...
On the \lowside{} host, record the \highside{} host's MAC:

\begin{lstlisting}[style=conf,caption={\file{/etc/ethers} on the LOW host}]
bb:bb:bb:bb:bb:bb 192.168.10.2
\end{lstlisting}

and on the \highside{} host, record the \lowside{} host's MAC:

\begin{lstlisting}[style=conf,caption={\file{/etc/ethers} on the HIGH host}]
...
\paragraph{nftables (the modern default).} On the \lowside{} host, egress
only:

\begin{lstlisting}[style=nft,caption={\file{/etc/nftables.conf} --- LOW host}]
#!/usr/sbin/nft -f
flush ruleset
...
On the \highside{} host, ingress only:

\begin{lstlisting}[style=nft,caption={\file{/etc/nftables.conf} --- HIGH host}]
#!/usr/sbin/nft -f
flush ruleset
...
never generates. Sniff \cmd{'ether proto 0x88b5'} on the \highside{} segment
with \cmd{tcpdump} while injecting one tagged frame on \lowside{}, then swap
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
  \node[host, minimum width=22mm] (low) {LOW host\\\scriptsize 192.168.10.1};
  ...
  \node[host, minimum width=22mm, right=14mm of B]
       (high) {HIGH host\\\scriptsize 192.168.10.2};
...
the \lowside{} network, links 2 and 3 become a complete return path from
\highside{} to \lowside{} and the diode is gone. Label that port, cover it, and
...
TP-Link\newline MC210CS & 1\,Gb/s   & single-mode & \nm{1310} & SC &
  Gigabit, up to 15\,km (check your revision). Cheap, commodity \nm{1310}
  splitters. \\
\bottomrule
\end{tabularx}
...
\begin{tipbox}[Keep a few optical attenuators on the bench]
These converters are built to drive hundreds of metres or kilometres, and a
diode's fibers are about a metre. Too much light at the receiver produces a
link that flaps or drops frames in ways that look like random loss. Fixed
attenuators (\dbb{3}, \dbb{5}, \dbb{10}) are inexpensive and fix it in
seconds.
\end{tipbox}
...
\item \textbf{Carrier independence.} Unplug MC-B's copper cable from the
      \highside{} host. MC-A must \emph{keep} its fiber link LED. If MC-A goes
      down, its carrier is coming from the far end and you have built a
      bidirectional link.

\item \textbf{Reverse probe.} On the \lowside{} host, start a capture that
      accepts everything:
\begin{lstlisting}[style=shell]
# LOW host -- must stay silent for the whole test
tcpdump -ni enp1s0 -e -vv
\end{lstlisting}
      From the \highside{} host, try hard to get a packet back:
\begin{lstlisting}[style=shell]
# HIGH host
...
\end{lstlisting}
      The \lowside{} capture must show \textbf{nothing} from the \highside{}
      host. Every command above must fail or time out.

\item \textbf{TCP impossibility.} From the \highside{} host,
...
\item \textbf{Forward path works.} From the \lowside{} host, send a datagram
      and see it arrive:
\begin{lstlisting}[style=shell]
# HIGH host
...
%% =====================================================================
\section{Run the daemons}

Start the \highside{} side first, so nothing is lost:
...
Point your senders at the amplifier. In \file{/etc/rsyslog.conf} on any host
that produces logs:
...
and on the \highside{} host, make rsyslog listen locally for what \deamp{}
re-emits:

\begin{lstlisting}[style=conf,caption={\file{/etc/rsyslog.d/10-diode-in.conf} --- HIGH host}]
module(load="imudp")
...
\subsection{The four options that matter}

\begin{table}[h]
\centering\small
\begin{tabularx}{\textwidth}{@{}llX@{}}
\toprule
\textbf{Option} & \textbf{Default} & \textbf{What it buys} \\
\midrule
\cmd{--batch N} & 1 &
  Packs $N$ log lines into one Reed--Solomon block. At \cmd{--batch 1} the
  code degenerates to plain repetition (the paper's scheme); $N \geq 2$ gives
  real coding gain. Costs: a decode failure loses the whole batch. \\
\cmd{--parity M} & 3 &
  $M$ parity fragments. Any $K$ of $K+M$ fragments rebuild the message.
  Raise it when the link is lossy. \\
\cmd{--interleave D} & 1 &
  Spreads the fragments of $D$ messages in time, so a burst cannot take every
  copy of one message. This is the fix for the paper's back-to-back repetition. \\
\cmd{--min-severity S} & 7 &
  Forward only severity $\leq S$. \cmd{--min-severity 4} ships warnings and
  worse. Unparseable lines are forwarded, not dropped. \\
\bottomrule
\end{tabularx}
\end{table}

Start at \cmd{--batch 8 --parity 6 --interleave 8}. If the \highside{} side
is missing messages, raise \cmd{--parity} first, then \cmd{--interleave}.
Handbook \S7.4 works through the tuning.

\subsection{As services}

\begin{lstlisting}[style=conf,caption={\file{/etc/systemd/system/ld-amp.service} --- LOW host}]
[Unit]
...
The \highside{} unit is the same shape with
\cmd{ExecStart=\ldots/ld\_deamp 6514 127.0.0.1 514 --key \$\{DIODE\_KEY\}}
and the identical key file, carried across by hand.
...
\item \textbf{Embedded and IoT.} Between two microcontrollers the diode is
      ...
      \S5.8 --- including why the \emph{sender} must be the bus master, and
      why I\textsuperscript{2}C can never be a diode.
...
\item \textbf{Why the daemons are proved.} The parser on the \highside{} side
      chews bytes chosen by whoever compromised the \lowside{} network, on a
      link with no way to ask for a retry --- Handbook \S7.1 and
